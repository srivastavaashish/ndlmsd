...
class iter_MLMDataset(Dataset):
    """Dataset class for masked language modeling."""

    def __init__(self, name, split, social_dim, data_dir):

        self.tok = BertTokenizer.from_pretrained('/home/ashish/Desktop/Desktop/bert-pretrained')
        
        
        # Change this directory when running prog on another comp   

        data = pd.read_csv('combined_data.csv', parse_dates=['time'])
        ...
        self.user2id, self.graph_data = iter_load_external_data(name, social_dim, data_dir)
...
def iter_load_external_data(name, social_dim, data_dir):
  

    with open('{}/{}_edges.p'.format(data_dir, name), 'rb') as f:
        edge_set = pickle.load(f)
        if name == 'reddit' or name == 'arxiv':
            edge_set = set(e[:2] for e in edge_set)
    ...
    user2id = {u: i for i, u in enumerate(filtered_users)}

    vectors = dict()

    with open('data/{}_train_vectors_{}.txt'.format(name,50), 'r') as f:
    ...
    vector_matrix = np.zeros((len(filtered_users), social_dim))

    for i, n in enumerate(filtered_users):
        vector_matrix[i, :] = vectors[n]

    x = torch.tensor(vector_matrix, dtype=torch.float)

    a = nx.adjacency_matrix(graph, nodelist=filtered_users)
    edge_index = torch.tensor(np.stack((a.tocoo().row, a.tocoo().col)).astype(np.int32), dtype=torch.long)

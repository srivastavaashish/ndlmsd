...
checkpoint_file = 'mlm_model_checkpoint.pt'
...
while start_row < len(train_df):
    ...
    # Get a random sample of 40% from all previous batches
    ...
    dataset_split_train = iter_MLMDataset(args.data, split='train', social_dim=50, data_dir=args.data_dir)
    collator1 = MLMCollator(dataset_split_train.user2id, dataset_split_train.tok)
    ...
    # model = MLMModel(
    #     n_times=dataset_split_train.n_times,
    #     social_dim=50,
    #     gnn='gat'
    # )
    ...
    for epoch in range(1, args.n_epochs + 1):
        ...
        for i, batch in enumerate(train_loader1):
            ...
            offset_t0, offset_t1, loss = model(labels, reviews, masks, segs, users, graph_data_train, times, vocab_filter_train)
    ...
    with open('{}/{}.txt'.format(args.results_dir, filename), 'a') as f:
        f.write('Train loss {:.3f},dev perplexity {:.3f}, test perplexity {:.3f}, Elapsed time: {:.2f} mins\n'.format(loss.item(),perplexity_dev,perplexity_test,elapsed_time))
        
    
    # Print the results
    print('Train loss {:.3f}, dev perplexity {:.3f}, test perplexity {:.3f}, Elapsed time: {:.2f} mins'.format(loss.item(), perplexity_dev, perplexity_test,elapsed_time))
...
model.load_state_dict(torch.load(os.path.join(checkpoint_dir, f'{checkpoint_file}.pt')))
perplexity = evaluate_model(model, test_loader, device)
print('Best Test perplexity: {:.4f}'.format(perplexity))

with open(f'{args.results_dir}/{filename}.txt', 'a') as f:
    f.write(f'Best Test perplexity: {perplexity:.4f}\n')     

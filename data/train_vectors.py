...
def main():
    ...
    with open('ciao_edges.p', 'rb') as f:
        edge_set = pickle.load(f)
    with open('ciao_users.p', 'rb') as f:
        users = pickle.load(f)

    assert len(edge_set) == len(edge_set)
    ...
    n2v_model.wv.save_word2vec_format('ciao_vectors_{}.txt'.format(args.dim), binary=False)
